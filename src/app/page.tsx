...
export default function Categorias() {
  ...
  useEffect(() => {
    const fetchMusicas = async () => {
      ...
      const categoriasUnicas = [...new Set(data.map((musica) => musica.categoria))];
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-5">
      ...
      <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 w-full max-w-5xl">
        {categorias.map((categoria) => {
          const categoriaUrl = removerAcentos(categoria).replace(/\s+/g, "-").toLowerCase();

          return (
            <li key={categoria} className="flex justify-center">
              <Link href={`/musicas/${categoriaUrl}`}>
